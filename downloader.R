...
## bases continuas - individual 2003:2019

bases <- get_microdata(year = 2003:2019, trimester = 1:4, type = 'individual')

bases <- bases %>% 
  filter(!(year==2003 & trimester %in% c(1,2)),
         !(year==2007 & trimester %in% c(3,4)),
         !(year==2015 & trimester %in% c(3,4)),
         !(year==2016 & trimester==1)) %>% 
...
bases <- bases %>% 
  mutate(microdata = map(microdata, 
                         function(x){
                           ...
                           if (!'CH15'%in% names(x))      {x$CH15 <- NA_character_}
                           if (!'CH16_COD'%in% names(x))  {x$CH16_COD <- NA_character_}
                           x %>% mutate(ADECCFR = as.character(.data$ADECCFR),
