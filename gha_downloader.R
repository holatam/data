...
gha_downloader <- function(universo = "individual"){
  ...
  if(universo == "individual"){
    
    bases_en_repo <- data.frame(
      anio = stringr::str_sub(list.files(glue::glue("eph/{universo}/")), start = 17, 20),
      trim = stringr::str_sub(list.files(glue::glue("eph/{universo}/")), start = 22, 22)) %>% 
      dplyr::mutate(
    ...
    tryCatch(
      ...
      expr = {
        ...
        print("Última base disponible en INDEC descargada")
      ...
      error = function(e){
        print("No hay nueva base en INDEC para descargar")
      }
    )
    ...
    bases_en_repo <- data.frame(
      anio = stringr::str_sub(list.files(glue::glue("eph/{universo}/")), start = 12, 15),
      trim = stringr::str_sub(list.files(glue::glue("eph/{universo}/")), start = 17, 17)) %>% 
      dplyr::mutate(
        periodo = paste0(anio, "_", trim),
    ...
    tryCatch(
      ...
      expr = {
        ...
        base_hog <- base_hog %>% 
          dplyr::as_tibble() %>% 
          dplyr::rename_all(toupper) %>% 
          dplyr::mutate(DECIFR = as.character(DECIFR),
                        IDECIFR = as.character(IDECIFR),
                        GDECIFR = as.character(GDECIFR),
                        PDECIFR = as.character(PDECIFR),
                        ADECIFR = as.character(ADECIFR),
...
gha_downloader("individual")
gha_downloader("hogar")
